...
class WikipediaCrawler(WikipediaConfig):
    def __init__(
        ...
        self.url = furl(url).set(scheme="https")
    ...
    def _get_logo(self):
        ...
        logo_url = furl(logo_elem.find("a")["href"]).set(scheme="https")
        file_ext = Path(logo_url.url).suffix.lower()
        if file_ext not in [".png", ".svg"]:
            return None

        raw_logo = requests.get(logo_url, headers=self.headers, stream=True)
        if raw_logo.status_code != 200:
            raise requests.HTTPError("Error downloading the image!")

        self.logo = raw_logo
        self.file_ext = file_ext

    def _export_logo(self):
